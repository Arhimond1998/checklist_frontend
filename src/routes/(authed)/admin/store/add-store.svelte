<script>
  ...
  async function onclick() {
    ...
    try {
      ...
      if (item.id_store) {
        resp = await bffPut(`api/stores/${item.id_store}`, postData);
      } else {
...
<div class="mx-auto max-w-md p-6">
  ...
  <div class="space-y-2">
    ...
    <Input bind:value={item.code} id="middleName" class="w-full" placeholder="Введите код" disabled={item.id_store !== null}/>
  </div>
