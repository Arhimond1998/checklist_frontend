<script>
  import { Input } from '$lib/components/ui/input';
  import { Button } from '$lib/components/ui/button';
  import { Label } from '$lib/components/ui/label';
  import * as Card from '$lib/components/ui/card';
  import { bffPost, bffPut } from '$lib/utils';
  import { goto } from '$app/navigation';
  import Page from '../+page.svelte';
  import RoleCombobox from '$lib/components/combobox/RoleCombobox.svelte';
  import ComponentCombobox from '$lib/components/combobox/ComponentCombobox.svelte';

  let isLoading = $state(false);

  let roleValues = $state([]);
  let componentValues = $state([]);

  async function onclick() {
    isLoading = true;
    try {
      for (const role of roleValues) {
        for (const component of componentValues) {
          const resp = await bffPost('api/role_components', { id_role: role, id_component: component });
        }
      }
      goto('/admin/role_component/list');
    } catch (error) {
      console.log({ error });
      alert(error.msg);
    } finally {
      isLoading = false;
    }
  }
</script>

<div class="mx-auto max-w-md p-6">
  <div class="space-y-2">
    <ComponentCombobox bind:values={componentValues}></ComponentCombobox>
  </div>

  <div class="space-y-2">
    <RoleCombobox bind:values={roleValues}></RoleCombobox>
  </div>

  <Button
    disabled={isLoading}
    {onclick}
    class="w-full bg-blue-600 hover:bg-blue-700 disabled:opacity-50"
  >
    {isLoading ? 'Сохранение...' : 'Сохранить'}
  </Button>
</div>
